...
engine = create_engine('postgresql://bookdb:hello@localhost/catalog')
    ...
    if request.method == 'POST':
        ...
        if 'provider' in login_session and login_session['provider']!= 'null':
            ...
            if (bookName and bookAuthor and coverUrl and description and bookCategory):
                ...
                state = new_state()
                return render_template("newItem.html", currentPage = "new", title = "Add New Book", errorMsg ="All Fields are Required!", state = state, login_session = login_session)
        else:
            state = new_state()
            books = queryAllBooks()
            ...
            state = new_state()
            return render_template("newItem.html", currentPage = "new", title = "Add New Book", state = state, login_session = login_session)
    else:
        state = new_state()
        books = queryAllBooks()
        return render_template('main.html', books = books, currentPage = 'main', state = state, login_session = login_session, errorMsg = "Please Login first to Add Book!")
        ...
        if 'provider' in login_session and login_session['provider']!= 'null':
            ...
            if book.user_id == user_id or user_id == admin_id:
                if (bookName and bookAuthor and coverUrl and description and bookCategory):
                    ...
                    state = new_state()
                    return render_template("editItem.html", currentPage = 'edit', title = "Edit Book Details", book = book, state = state, login_session = login_session, errorMsg = "All Fields are Required!")
            else:
                state = new_state()
                return render_template("itemDetail.html", book = book, currentPage ='detail', state = state, login_session = login_session, errorMsg = "Sorry! The Owner can only edit book Details!")
        else:
            state = new_state()
            return render_template("itemDetail.html", book = book, currentPage ='detail', state = state, login_session = login_session, errorMsg = "Please Login to Edit the Book Details!")
    elif book:
        state = new_state()
        if 'provider' in login_session and login_session['provider']!= 'null':
            user_id = check_user().id
...
def deleteBook(category, bookId):
    ...
    if book:
        if 'provider' in login_session and login_session['provider']!= 'null':
            ...
            return render_template("itemDetail.html", book = book, currentPage ='detail', state = state, login_session = login_session, errorMsg = "Please Login to Delete the Book!")
    else:
        return render_template("main.html", currentPage = 'main', error = 'Error Deleting Book! No Book Found with this Category and Book Id :(', state = state, login_session = login_session)
...
def gConnect():
    ...
    if not check_user():
        createUser()
    return jsonify(name=login_session['name'], email=login_session['email'],
